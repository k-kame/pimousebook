...
よって，129 行目からプログラムを読んでいきましょう．

 * 129行目：python のお約束で，runmouse_base.py が直接呼ばれたときは 130-136行が実行され，他から import された場合には，130-136行が実行されず，class RunMouse() のみ参照されるというものです．
 * 131行目：RunMouse ノードを生成して
 * 133行目：launch で設定した parameter を変数 sim_act に読み込みます．今後，sim_act の値で，実機用／シミュレーション用に動作を分岐させます．
